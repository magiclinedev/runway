    ...
    <div class="container mx-auto">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-4 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <a v-if="tableData_Collection.length > 0" :key="tableData_Collection[0].id" class="col-span-3 bg-gray-100 rounded-md overflow-hidden" :style="{ backgroundImage: `url('/storage/${encodeURIComponent(tableData_Collection[0].image)}')`, backgroundPosition: '50% 0%', backgroundSize: 'cover' }" :href="`/collection_view/${encodeURIComponent(tableData_Collection[0].id)}`">
                ...
                <a v-if="tableData_Collection.length > 1" :key="tableData_Collection[1].id" class="row-span-2 col-span-2 bg-gray-100 rounded-md overflow-hidden" :style="{ backgroundImage: `url('/storage/${encodeURIComponent(tableData_Collection[1].image)}')`, backgroundPosition: '50% 0%', backgroundSize: 'cover' }" :href="`/collection_view/${encodeURIComponent(tableData_Collection[1].id)}`">
                ...
                <a v-for="(item, index) in tableData_Collection.slice(2)" :key="item.id" class="bg-gray-100 rounded-md overflow-hidden" :style="{ backgroundImage: `url('/storage/${encodeURIComponent(item.image)}')`, backgroundPosition: '50% 0%', backgroundSize: 'cover' }" :href="`/collection_view/${encodeURIComponent(item.id)}`">
